...
MyAssimpInstanceLoader::MyAssimpInstanceLoader()
{
}
...
MyOpenGL::MyObject* MyAssimpInstanceLoader::load(const std::string& path, int instanceCount) {
	//拿出模型所在目录
...
//每个节点对应一个object
void MyAssimpInstanceLoader::processNode(aiNode* ainode, MyOpenGL::MyObject* parent, const aiScene* scene, const std::string& rootPath, int instanceCount) {
...
////处理mesh
MyOpenGL::MyInstancedMesh* MyAssimpInstanceLoader::processMesh(aiMesh* aimesh, const aiScene* scene, const std::string& rootPath, int instanceCount) {
...
MyOpenGL::MyTexture* MyAssimpInstanceLoader::processTexture(
...
glm::mat4 MyAssimpInstanceLoader::getMat4f(aiMatrix4x4 value) {
	glm::mat4 to(
